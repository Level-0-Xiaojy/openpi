"""
V6: Based on V5, adds `action_frequency` as the last dimension of actions.

Changes vs V5:
- `action_frequency` is appended to ACTION_KEYS, so action dim = 28 -> 29.
- `load_json_data` reads per-frame `action_frequency` from JSON, defaulting to 20Hz
  if the field is missing (backward compatible with un-annotated JSONs).
...
def load_json_data(episode_path: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        for key in all_keys:
            if key == 'action_frequency':
                trajectories[key].append(
                    frame_data.get(key, DEFAULT_ACTION_FREQUENCY)
                )
            else:
    ...
    for key, vals in trajectories.items():
        ...
        if 'gripper' in key or key == 'action_frequency':
            arr = arr.reshape(-1, 1)
        arrays[key] = arr
