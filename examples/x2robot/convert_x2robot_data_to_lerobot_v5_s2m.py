"""
V5 (s2m variant): Direct ffmpeg transcoding for s2m (Slave-to-Master) training.

State = follow_* (slave), 14-D
Action = master_* (master), 14-D

Optimization strategy (same as v5):
...
class ConvertArgs:
    raw_paths: list[str] = dataclasses.field(
        default_factory=lambda: ['/mnt/public/zhangjiahao2/striding/processed_data/'],
    )
    repo_name: str = "fold_towel_s2m"
...
# State keys: follow_* fields (slave), 14-D total
STATE_KEYS = [
...
# Action keys: master_* fields (master), 14-D total
ACTION_KEYS = [
...
def load_json_data(episode_path: str) -> tuple[np.ndarray, np.ndarray]:
    """Load and parse JSON data for s2m.

    state_array: follow_* at frame i, shape (num_frames-1, STATE_DIM)
    action_array: master_* at frame i+1, shape (num_frames-1, ACTION_DIM)
    """
    ...
    # Build state array: follow_* (slave)
    follow_arr = np.empty((num_frames, STATE_DIM), dtype=np.float32)
    for i, frame in enumerate(data):
        offset = 0
        for key in STATE_KEYS:
            val = frame[key]
            if key.endswith('gripper'):
                follow_arr[i, offset] = float(val)
                offset += 1
            else:
                follow_arr[i, offset:offset + 3] = val
                offset += 3

    # Build action array: master_* (master)
    action_arr = np.empty((num_frames, ACTION_DIM), dtype=np.float32)
    for i, frame in enumerate(data):
        offset = 0
        for key in ACTION_KEYS:
            val = frame[key]
            if key.endswith('gripper'):
                action_arr[i, offset] = float(val)
                offset += 1
            else:
                action_arr[i, offset:offset + 3] = val
                offset += 3

    # Align: state[i] -> action[i+1], producing num_frames-1 pairs
    return follow_arr[:-1], action_arr[1:]
...
def transcode_video_ffmpeg(
    ...
    """Direct ffmpeg transcoding, matching encode_video_frames parameters."""
...
def transcode_single_video(
    ...
    """Transcode a single video file. Returns (episode_idx, camera_name, num_frames)."""
...
def get_dummy_video_stats(num_frames: int) -> dict:
    """Generate dummy video feature statistics."""
    return {
...
def compute_episode_stats_with_dummy_video(
    ...
    """Compute episode stats, using dummy values for video features."""
...
def main(args: ConvertArgs):
    """
    V5 (s2m): Direct ffmpeg transcoding for Slave-to-Master training.
    State = follow_* (14-D), Action = master_* (14-D).
    """
    print(f"HF_LEROBOT_HOME: {HF_LEROBOT_HOME}")
    print(f"V5 (s2m): Direct ffmpeg transcoding")
    print(f"  State: {STATE_DIM}-D (follow_*/slave) | Action: {ACTION_DIM}-D (master)")
    print(f"  fps={args.fps}, num_workers={args.num_workers}")
    ...
    for ep_idx, ep_path in enumerate(episode_paths):
        for camera_name, video_filename in FILE_CAMERA_MAPPING.items():
            transcode_tasks.append((ep_path, ep_idx, camera_name, video_filename, output_path, target_size, args.fps))
    ...
    for ep_idx, ep_path in tqdm.tqdm(enumerate(episode_paths), total=len(episode_paths), desc="Building dataset"):
        ...
        num_pairs = len(state_array)  # = num_frames - 1

        dataset._video_frame_count = num_pairs
