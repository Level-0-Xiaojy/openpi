...
def detect_intervention_start(arrays: dict) -> int:
    """
    功能 2: 寻找模型Rollout后的第一次长静止期，返回人类接管的起始索引。
    如果在数据中找到第一次连续 >= PAUSE_FRAMES (默认80帧) 的静止窗口，
    且紧接着出现了运动帧，则认为该运动帧是人类接管的开始。
    """
    ...
    for i in range(n_frames):
        if not is_moving[i]:
            ...
            # 如果动了，检查之前是不是刚刚经历了一个足够长的静止期
            if current_pause_len >= PAUSE_FRAMES:
                # 找到了！这就是第一次人工接管的起点
                return i
            # 如果之前的静止期不够长（比如只是稍微顿了一下），清零重新算
            current_pause_len = 0
            
    # 如果整个过程中都没有出现过 >= 4秒的静止期并重新启动，默认保留全部数据
    return 0
...
def process_episode(episode_dir: str, output_dir: str, enable_takeover_detect: bool, enable_stationary_filter: bool):
    ...
    print(f"  [{ep_name}] Total: {total_frames} -> Kept: {len(keep_indices)} "
          f"(Drop Pre-Intervention: {drop_rollout}, Drop Stationary: {drop_stationary})")
