...
def find_h5_episode_files(data_dir: str) -> List[str]:
    ...
    if not episode_files:
        episode_files = sorted(glob.glob(os.path.expanduser(os.path.join(data_dir, "episode_*", "*.h5"))))
...
def process_episode_h5(h5_path: str, instruction: str) -> Optional[Tuple[str, int, Generator[Dict[str, Any], None, None]]]:
    """
    ...
    try:
        ...
        def frame_generator():
            ...
            for step_idx in range(num_steps):
                ...
                action_gripper = action_full_vec[6:] # [0, 1]
...
def main(repo_id: str, data_dir: str, instruction: str, *, push_to_hub: bool = False, max_episode_num: Optional[int] = None):
    ...
    dataset = LeRobotDataset.create(
        ...
        fps=10, # TODO: Adjust FPS if needed
        features={
            # For gr00t (C, H, W)
    ...
    for idx, h5_path in enumerate(tqdm(episode_files, desc="Processing Episodes"), 1):
        processed_data = process_episode_h5(h5_path, instruction)
