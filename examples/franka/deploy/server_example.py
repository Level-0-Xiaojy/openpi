...
class OpenVLAServer:
    ...
    def predict_action(self, payload: Dict[str, Any]) -> str:
        try:
            ...
            image_array, instruction = payload["images"], payload["instruction"]
            ...
            if len(image_array.shape) == 3:
                ...
                for i in range(image_array.shape[0]):
                    ...
                    plt.imshow(image)
                    plt.axis("off")
                    plt.title(f"Camera {i} (Press [X] to continue)")
                    plt.show()
...
def parse_args():
    ...
    parser.add_argument('--openvla_path', type=str, default="openvla/openvla-7b", help="HF Hub Path or local run directory")
    parser.add_argument('--host', type=str, default="0.0.0.0", help="Host IP Address")
    parser.add_argument('--port', type=int, default=9876, help="Host Port")
